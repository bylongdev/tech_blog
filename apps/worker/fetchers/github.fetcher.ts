...
export class GitHubFetcher implements IFetcher {
	readonly sourceName = "GitHub Blog";
	readonly sourceSlug = "github";
	...
	// Fetch articles from the GitHub blog RSS feed
	async fetch(): Promise<CreateRawArticleDto[]> {
		try {
			const source = await this.getSource();

			// Use the rss-parser library to fetch and parse the RSS feed from GitHub
			const feed = await parser.parseURL(
				source.url || "https://github.blog/feed/",
			);
